...
<div class="modal fade" id="createInspectionModal" tabindex="-1" aria-labelledby="createInspectionModalLabel"
    aria-hidden="true">
                ...
                @if (app()->getLocale() == 'ar')
                    <div class="d-flex justify-content-between align-items-center w-100">
                        <h5 class="modal-title" id="createInspectionModalLabel">
                            {{ __('messages.create_new_inspection') }}
                        </h5>
                ...
                <form id="createInspectionForm" class="protected-form" novalidate>
                    ...
                    <div class="mb-3">
                        <label for="checklist_items"
                            class="form-label fw-medium">{{ __('messages.checklist_items') }}</label>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" class="form-control Input_control" id="images" name="images[]" multiple
  ...
  `;
            ...
            createInspectionModal.addEventListener('shown.bs.modal', async function() {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        const categoryWrapper = categorySelect?.closest(
                            '.searchable-dropdown');
                        const categoryInput = categoryWrapper?.querySelector(
                            'input[type="text"]');

                        if (categoryInput && !categoryInput.hasAttribute(
                                'data-custom-listener')) {
                            ...
                            categoryInput.addEventListener('keydown', function(e) {
                                if (e.key === 'Enter' && this.value.trim()) {
                                    ...
                                    const existingOption = Array.from(categorySelect
                                        .options).find(opt =>
                                        opt.value.toLowerCase() === newValue
                                        .toLowerCase()
                                    );
                                    ...
                                    if (!existingOption) {
                                        ...
                                        if (categorySelect.searchableDropdown) {
                                            categorySelect.searchableDropdown
                                                .updateOptions();
                                        }
                                    }

                                    // Select the option
                                    categorySelect.value = existingOption ?
                                        existingOption.value : newValue;
                                    this.value = existingOption ? existingOption
            ...
            createInspectionForm.addEventListener('submit', function(e) {
                ...
                if (fileInput.files && fileInput.files.length > 0) {
                    ...
                    openDrawingModal({
                        ...
                        saveButtonText: '{{ __('messages.submit_inspection') }}',
                        mode: 'image',
                    ...
                    document.getElementById('drawingModal').addEventListener('shown.bs.modal',
                        function() {
    ...
    async function submitInspectionWithoutImages() {
        try {
            ...
            if (response.code === 200) {
                const inspectionModal = bootstrap.Modal.getInstance(document.getElementById(
