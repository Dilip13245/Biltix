        ...
        <div class="card B_shadow">
          <div class="border-bottom pb-3 px-md-4 px-2 py-3 py-md-4">
            <h5 class="fw-semibold black_color mb-3">
              <svg width="18" height="19" class="{{ margin_end(1) }}" viewBox="0 0 18 19" fill="none"
          ...
          <div class="card-body p-md-4">
            <form id="supportForm">
              @csrf
              <div class="mb-3">
                <label for="fullName" class="form-label fw-medium">{{ __('messages.full_name') }}</label>
                <input type="text" class="form-control Input_control" id="fullName" name="full_name" required
                  placeholder="{{ __('messages.enter_full_name') }}" style="{{ is_rtl() ? 'text-align: right;' : '' }}">
              </div>
              ...
              <div class="mb-3">
                ...
                <input type="email" class="form-control Input_control" id="email" name="email" required
                  placeholder="{{ __('messages.enter_email_address') }}" style="{{ is_rtl() ? 'text-align: right;' : '' }}">
              </div>
      ...
      try {
        ...
        if (response.code === 200) {
          ...
          if (typeof toastr !== 'undefined') {
            toastr.success(response.message || '{{ __('messages.support_request_submitted') }}');
          } else {
            alert(response.message || '{{ __('messages.support_request_submitted') }}');
          }
          supportForm.reset();
        } else {
          // Error
          if (typeof toastr !== 'undefined') {
            toastr.error(response.message || '{{ __('messages.failed_to_submit_request') }}');
          } else {
            alert(response.message || '{{ __('messages.failed_to_submit_request') }}');
          }
        }
      } catch (error) {
